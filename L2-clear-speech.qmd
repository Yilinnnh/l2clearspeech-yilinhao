...
write_csv(data_all, "cleaned_data.csv")
...
for (m in measures) {
  ...
  data_m <- data_long %>% 
    filter(measure == m) %>%
    mutate(
      group = factor(group, levels = c("L2_diff_L1", "native", "L2_same_L1")),
...
# Results
L2 speakers with different L1 had a significantly higher F0_range
